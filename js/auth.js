...
async function handleLogin() {
  ...
  if (!email || !pass) {
    if (errBox) { errBox.innerText = '⚠️ Email mathu Password eradu enter maadi!'; errBox.classList.remove('hidden'); }
    return;
  }
...
function switchTab(tab) {
  ['dashboard', 'payouts', 'users', 'tasks', 'settings'].forEach(t => {
    ...
    if (content && nav) {
      if (t === tab) { content.classList.remove('hidden'); nav.classList.add('active-nav'); nav.classList.remove('text-gray-400'); }
      else { content.classList.add('hidden'); nav.classList.remove('active-nav'); nav.classList.add('text-gray-400'); }
    }
  });
}
